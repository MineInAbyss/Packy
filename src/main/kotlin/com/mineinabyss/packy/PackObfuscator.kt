...
class PackObfuscator(private val resourcePack: ResourcePack) {
    ...
    private fun obfuscateModels() {
        ...
        obfuscatedModels.forEach {
            resourcePack.removeModel(it.originalModel.key()).takeIf { b -> it.originalModel.key().asString().contains("bonfire")}?.logVal("${it.originalModel.key().asString()}: ")
    ...
    private fun obfuscateModel(model: Model) =
        obfuscatedModels.findObf(model.key()) ?: model.obfuscateModelTextures().obfuscateOverrides().toBuilder().key(
            model.key().obfuscateKey().takeUnless { VanillaKeys.isVanilla(model) } ?: model.key()
        ).build()
    ...
    private fun Model.obfuscateOverrides(): Model = obfuscatedModels.findObf(key()) ?: toBuilder().overrides(
        overrides().filterNotNull().map { override ->
            ...
            val modelKey = obfuscatedModels.findObf(override.model())?.key()
                ?: resourcePack.takeUnless { override.model() == this.key() }?.model(override.model())?.let(::obfuscateModel)?.key()
                ?: override.model()

            return@map ItemOverride.of(modelKey, override.predicate())
        }
    ).key(key().obfuscateKey()).build().also { obfuscatedModels += ObfuscatedModel(this@obfuscateOverrides, it) }
    ...
    private fun Key.obfuscateKey() = when (packy.config.obfuscation) {
        ...
        PackyConfig.ObfuscationType.SIMPLE -> Key.key(UUID.randomUUID().toString())
